...
class Main
{
    ...
    private function events()
    {
        if (isset($_REQUEST['buy']) OR $_REQUEST['buy'] == 1)
        {
            ...
            $a = $this->buy($nick, $group, $serv_id, $promo);
...
public function buy($name, $group_id, $srv_id, $promo)
{
    $name = trim($name);
    ...
    if ($result) {
        ...
        return $this->pay->redirect($this->db->mysqli->insert_id.'_'.$group['name'].'_'.$srv_id, $price_, $group['name'], $promo, $name);
    } else {
